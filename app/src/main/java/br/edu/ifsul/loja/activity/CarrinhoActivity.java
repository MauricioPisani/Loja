...
public class CarrinhoActivity extends AppCompatActivity {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()){
            ...
            case R.id.menuitem_cancelar_pedido:
               for (int i=0;i<=AppSetup.carrinho.size();i++) {
                   devolverProduto((ItemPedido) item);
               }
    ...
    //rollback no estoque
public void devolverProduto(ItemPedido item){
    final DatabaseReference myRef = FirebaseDatabase.getInstance().getReference().child("vendas/produtos/" + item.getProduto().getKey() + "/quantidade");
